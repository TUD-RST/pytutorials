...
at the top of the code used in the previous sections. Under Windows, it might be necessary to explicitly specify the path to the \emph{FFMPG} library, e.g.:
\begin{lstlisting}
...
Note that \code{fraw_the_car} is in the scope of the \code{car_animation} function and, hence, has full access to the handle \code{h_car} defined there.
