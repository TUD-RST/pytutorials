...
\luainputlistingline{../sim/02_car_example_animation.py}{import matplotlib}
...
Two further functions are required: \texttt{init()} and \texttt{animate(i)}. They will be called later by \mpl to initialize and perform the animation. The \texttt{init()}-function defines which objects change during the animation, in this case the two axes the handles of which are returned:
